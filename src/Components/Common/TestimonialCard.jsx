...
const TestimonialCard = ({ image, name, role, title, description }) => {
  ...
  return (
    <div
      className={`bg-[#3124461A] backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-lg text-left flex flex-col hover:scale-[1.02] transition-transform duration-300`}
    >
      {/* User Info */}
      <div className="flex items-center mb-4">
        <img
      ...
      <div
        className={`text-base leading-relaxed  transition-all duration-300 ${
          expanded ? "max-h-[500px]" : "max-h-[80px] overflow-hidden"
        }`}
      >
        {description}
      </div>
      ...
      {description.length > 120 && (
        <button
          ...
          className="text-sm text-purple-400 mt-2 self-start hover:underline"
        >
