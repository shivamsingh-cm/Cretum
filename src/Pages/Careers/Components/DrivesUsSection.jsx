        ...
        <div className="flex flex-col md:flex-row items-start gap-10">
              ...
              return (
                <div
                  ...
                  className={`border rounded-lg overflow-hidden transition-all duration-300 shadow-sm ${
                    isActive
                      ? "border-[#A164FF] bg-gray-50 shadow-md"
                      : "border-gray-200 bg-white hover:border-gray-300"
                  }`}
                >
                  <button
                    className="w-full flex justify-between items-center px-5 py-4 text-left font-semibold text-lg sm:text-xl focus:outline-none transition-colors"
                    onClick={() => setActiveId(item.id)}
                  >
                    <span
                      className={`${
                    ...
                    <span
                      className={`text-[#A164FF] text-2xl font-light transform transition-transform duration-300 ${
                        isActive ? "rotate-0" : ""
                      }`}
                    >
                      {isActive ? "−" : "+"}
                  ...
                  {isActive && (
                    <div className="px-5 pb-5 text-gray-600 text-sm sm:text-base leading-relaxed transition-all duration-300">
                      {item.discription}
                    </div>
                  )}
                </div>
          ...
          <div className="w-full md:w-1/2 flex justify-center md:justify-end">
            <img
              src={driveUs.find((item) => item.id === activeId)?.image}
              alt={driveUs.find((item) => item.id === activeId)?.title}
              className="w-full max-w-md h-96 object-cover rounded-lg shadow-lg border border-gray-200 transition-all duration-500 ease-in-out"
              loading="lazy"
            />
          </div>
        </div>
