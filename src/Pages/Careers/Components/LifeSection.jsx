  ...
  return (
    <section className="w-full bg-white py-16 px-6 sm:px-12 lg:px-24">
      ...
      <div className="max-w-7xl mx-auto">
        {/* Section Heading */}
        <div className="mb-12 text-center sm:text-left">
          <h1 className="text-base text-[#A164FF] mb-4 font-medium">
            Life at Cretum
          </h1>
          <p className="text-gray-600 text-sm sm:text-lg md:text-2xl max-w-7xl font-medium mx-auto sm:mx-0 leading-relaxed">
            Work here isn't about clocking in and out — it's about contributing
            to something bigger. You'll join a team that celebrates wins
            together, supports each other through challenges, and enjoys the
            ride along the way.
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {life.map((item) => (
            <div
              key={item.id}
              className="relative w-full h-80 rounded-xl overflow-hidden shadow-lg group border border-backgroundPrimary/50 hover:shadow-xl transition-all duration-300"
            >
              <img
                src={item.image}
                alt={item.title}
              ...
              {/* Gradient overlay with white background */}
