  ...
  const cardsTop = [
    {
      ...
      desc: "Reconciliation errors can lead to significant ITC losses and cash flow issues.",
      Icon: TrendingDown,
    },
    {
      title: "Confused About Sector-Specific Rules?",
      desc: "Complex industry rules and composition-specific deviations create compliance risk.",
      Icon: FileSearch,
    },
    {
      title: "Drowning in GST Notices?",
      desc: "Improper filings increase legal exposure and penalties.",
      Icon: AlertCircle,
    },
    {
      title: "Manual Reconciliation Errors Costing Lakhs?",
      desc: "Manual workflows lead to duplicate efforts, missed deadlines & interest.",
      Icon: Layers,
    },
  ];
    ...
    <div className="w-full bg-backgroundPrimary py-20">
      <div className="max-w-[1200px] mx-auto px-6 text-center">
        ...
        <p className="text-gray-400 mt-3 text-base max-w-[640px] font-normal mx-auto">
          Businesses face numerous GST complexities. We’ve helped hundreds
          overcome these exact challenges.
        </p>

        {/* ===== section first top cards ===== */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
          {cardsTop?.map((c, i) => (
            ...
            >
              ...
              <h3 className="text-white font-medium text-lg text-card-title leading-snug">
                {c?.title}
              </h3>
        ...
        {/* ===== section second bottom cards with bullets ===== */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-12">
          {cardsBottom?.map((c, i) => (
            <article
              key={i}
              ...
              <ul className="mt-4 text-gray-300 text-card-small space-y-2">
                {c?.points?.map((pt, idx) => (
                  <li key={idx} className="flex items-start gap-2">
                    <CheckCircle
                      className="text-buttonBackground flex-shrink-0"
                      size={18}
                    />
                    <span>{pt}</span>
                  </li>
