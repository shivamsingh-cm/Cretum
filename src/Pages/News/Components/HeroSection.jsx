  ...
  useEffect(() => {
    const fetchNewsData = async () => {
      try {
        ...
        const featured =
          newsApi.getFeaturedNews()[0] || newsApi.getLatestNews()[0];
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="text-center mb-6 sm:mb-10 mt-16">
          ...
          <p className="text-white font-normal text-base sm:text-sm md:text-lg max-w-3xl mx-auto mt-4">
            Discover the latest updates, announcements, and insights shaping our
            journey and the businesses we serve.
          </p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          {newsData?.featuredNews?.map((article) => (
            <div
              key={article.id}
              className="lg:col-span-2 bg-backgroundSecondary rounded-3xl p-4 sm:p-6 shadow-xl border border-backgroundSecondary flex flex-col"
            >
              <img
          ...
          <div className="lg:col-span-1 flex flex-col gap-4 sm:gap-6">
            {newsData.latestNews.map((article) => (
              <div
                key={article.id}
                className="bg-backgroundSecondary rounded-3xl p-4 sm:p-5 shadow-xl border border-backgroundSecondary flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-start gap-3 sm:gap-4"
              >
                <img
                  src={getImageUrl(article.imageUrl)}
                  alt={article.title}
